...
function calculateMaterials(data) {
    try {
        logger_1.logger.debug({ context: 'Calculator', message: 'Starting calculation', data });
        let dowels = 0;
        let screws = 0;
        let klopScrews = 0;
        let wagoTerminals = 0;
        ...
        logger_1.logger.debug({ context: 'Calculator', message: 'Calculation finished successfully', result });
        return result;
    }
    catch (error) {
        logger_1.logger.error({ context: 'Calculator', message: 'Failed to calculate materials', stack: error instanceof Error ? error.stack : undefined });
        throw error;
    }
}
